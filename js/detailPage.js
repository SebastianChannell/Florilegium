...
function highlightOrdoLabels(line){return escapeHtml(line).replace(/\b(Office|Matins|Lauds|Prime|Terce|Sext|None|Vespers|Compline):/g,'<span class="ordo-label-highlight" style="color:var(--sf-purple);font-style:italic;">$1:</span>');}
...
function commemorationPrayer(value,label){const tag='Commemor'+'ation:';return String(value || '').split(new RegExp(`\\n(?=${tag})`,'i')).map((block)=>{const title=(block.match(new RegExp(`^${tag}\\s*(.+)$`,'im')) || [,''])[1];const match=block.match(new RegExp(`(?:^|\\n)${label}\\n([\\s\\S]*?)(?=\\n(?:Oratio|Secreta|Postcommunio)\\n|$)`,'i'));return match ? [`Commemoratio${title ? `: ${title}` : ''}`,match[1].trim()].filter(Boolean).join('\n') : '';}).filter(Boolean).join('\n\n');}
function withCommemoration(properText,commemorationText){return [properText,commemorationText].filter((value)=>String(value || '').trim()).join('\n\n');}
function renderPropers(propers){return `${entry('Introitus',propers.introit)}${entry('Oratio',withCommemoration(propers.collect,commemorationPrayer(propers.commemorations,'Oratio')))}${entry('Lectio',propers.epistle)}${entry('Graduale',propers.gradual)}${entry('Alleluia',propers.alleluia)}${entry('Tractus',propers.tract)}${entry('Evangelium',propers.gospel)}${entry('Offertorium',propers.offertory)}${entry('Secreta',withCommemoration(propers.secret,commemorationPrayer(propers.commemorations,'Secreta')))}${entry('Praefatio',propers.preface)}${entry('Communio',propers.communion)}${entry('Postcommunio',withCommemoration(propers.postcommunion,commemorationPrayer(propers.commemorations,'Postcommunio')))}`;}
